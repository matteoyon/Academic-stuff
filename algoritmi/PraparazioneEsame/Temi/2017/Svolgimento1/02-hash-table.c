...
List è utilizzata per strutturare la hashtable: quest'ultima viene strutturata infatti come un array
di List, ognuna delle quali corrisponde ad un possibile valore restituito dalla funzione di hash. Viene sfruttata
la List in modo da poter gestire le collisioni, aggiungendo eventualmente un elemento in testa alla List corrispondente.
...
 
2. Se t è una variabile di tipo Dict, cosa rappresenta t -> array[3] ?
Corrisponde alla lista contenente tutti gli elementi con valore di hashing della chiave pari a 3.
...
