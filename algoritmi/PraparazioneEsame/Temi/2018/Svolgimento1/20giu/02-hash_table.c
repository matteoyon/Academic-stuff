...
La list viene usata per gestire le collisioni nella hash table.

2. Se t è una variabile di tipo Dict, cosa rappresenta t -> array[3] ?
 
INSERIRE LA RISPOSTA
Rappresenta la lista degli elementi che hanno valore di hash pari a 3
 
3. Cosa succede se si tenta di inserire nella tabella di hash
un Item con una chiave già presente nella tabella?
 
INSERIRE LA RISPOSTA
L'Item viene inserito nella lista corrispondente alla valore di hash della chiave.
